...
public class RadarSystem : MonoBehaviour
{
	...
	void Update ()
	{
		...
		case Alignment.Center:
			inposition = new Vector2((Screen.width / 2) - (Size / 2), (Screen.height / 2) - (Size / 2)) + PositionOffset;
			break;
		}
		...
		if (Input.GetKeyDown(KeyCode.Escape))
		{
			Size = 500;
			Scale = 3f;
			PositionAlignment = Alignment.RightBot;
		}
	}
	...
	void OnGUI ()
	{
		...
		if (CompassBackground)
			...
			GUI.DrawTexture (new Rect (inposition.x + 50, inposition.y + 50, backgroundSize, backgroundSize), CompassBackground);
		GUIUtility.RotateAroundPivot ((this.transform.eulerAngles.y), inposition + new Vector2 (Size / 2f, Size / 2f)); 
		if (NavCompass)
